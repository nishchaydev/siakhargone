    ...
    <header className={headerClasses}>
      ...
      <div className="container mx-auto flex h-[70px] max-w-7xl items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-3">
          <span className={cn("font-bold font-headline text-xl", linkColor)}>
            ...
            {navItems.map((item) => (
              <NavigationMenuItem key={item.title}>
                {item.children ? (
                  <>
                    <NavigationMenuTrigger 
                      className={cn(navigationMenuTriggerStyle(), "bg-transparent font-sans font-semibold", linkColor, `hover:text-accent focus:bg-accent/10`,
                  ...
                  <NavigationMenuLink asChild>
                     <Link
                        ...
                        className={cn(navigationMenuTriggerStyle(), "bg-transparent font-sans font-semibold", linkColor, `hover:text-accent`,
                        ((isHomePage && activeSection && item.href.includes(activeSection)) || pathname === item.href) ? `font-bold text-primary` : ""
                      )}
                    >
                      {item.title}
                    </Link>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className={cn("lg:hidden", `${linkColor} hover:${linkColor} hover:bg-muted`)}
